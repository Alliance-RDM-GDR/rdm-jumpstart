...
Consult the project's file naming and folder structure conventions ([Day3](Day03_Lesson3.html)) if needed.
...
If you can’t remember how this function works, look back to your practice scripts, or review [Day 3](Day03_Lesson4.html).
...
Was the data imported correctly? Use some of the functions you learned in [Day 3](Day03_Lesson4.html) to check.
...
Note: In this script, we are going to create a new object every time we perform an action on the dataset (such as, `survey_data_fixnames` or `survey_data_nodupl`). We do this to keep a visual record of the changes we are making to the dataset. However, when working on your own projects, avoid creating too many similar objects in your working environment. Instead, you will [overwrite the original object](Day02_Lesson4.html) with the changes implemented (e.g., always overwrite the object named `survey_data` without adding the description).
